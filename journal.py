...
def get_file_lock(lockfile: str):
...
def create_operation_log(op_code):
...
class JournalStorage(BaseStorage):
    ...
    **Data persistence**
    ...
    def _buffer_log(self, log=Dict[str, Any]):
    ...
    def _apply_log(self, log: Dict[str, Any]):
        op = log["op_code"]
    ...
    def create_new_study(self, study_name: Optional[str] = None) -> int:
        ...
        """
        lock = get_file_lock("create_new_study")
        lock.acquire()
        self._sync_with_backend()
        if study_name in [s.study_name for s in self._studies.values()]:
            lock.release()
            raise DuplicatedStudyError
        log = create_operation_log(JournalOperation.CREATE_STUDY)
    ...
    def get_trial_param(self, trial_id: int, param_name: str) -> float:
        ...
        return trial.distributions[param_name].to_internal_repr(
